...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            inkos_desktop::plugin::commands::cmd_broadcast_event,
        ])
...
///
/// 窗口创建/聚焦逻辑抽在 `plugin::commands::open_manager_window`，托盘菜单
/// 「插件管理…」也复用同一入口，避免两处逻辑漂移。
#[tauri::command]
fn cmd_open_plugin_manager(app_handle: tauri::AppHandle) -> Result<(), String> {
    inkos_desktop::plugin::commands::open_manager_window(&app_handle)
}
