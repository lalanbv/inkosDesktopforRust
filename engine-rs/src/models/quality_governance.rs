...
impl QualityVerdict {
    ...
    /// 判定 → 是否记债（债务账本的入账口径）。
    /// 双端消费面：TS scheduler.ts verdictCreatesDebt；Rust ops_routes
    /// handle_audit_failure（540 号 daemon 判定化接线清偿）。
    ...
    /// 双端消费面：TS scheduler.ts verdictContinuesPipeline；Rust ops_routes
    /// handle_audit_failure（540 号 daemon 判定化接线清偿）。
