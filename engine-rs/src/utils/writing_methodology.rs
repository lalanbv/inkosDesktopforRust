//! 写作方法论完整版（style_guide.md 注入用）。
//!
//! 移植自 `packages/core/src/utils/writing-methodology.ts`（164 行）。
//! initBook/generateStyleGuide 时注入一次，writer 每章读取 style_guide 上下文。
//! 系统提示词里的紧凑"craft card"是它的摘要——**逐字移植，零改写**。

use crate::utils::language::WritingLanguage;

/// 按语言选择方法论全文。
pub fn build_writing_methodology_section(language: WritingLanguage) -> String {
    if language == WritingLanguage::En {
        build_english_methodology()
    } else {
        build_chinese_methodology()
    }
}

fn build_chinese_methodology() -> String {
    r#"---
...
8. 本章是否推进了主线目标？"#.to_string()
}

fn build_english_methodology() -> String {
    r#"---
...
8. Does this chapter advance the main plotline?"#.to_string()
}
...
mod tests {
    ...
    #[test]
    fn methodology_bilingual_skeleton() {
        let zh = build_writing_methodology_section(WritingLanguage::Zh);
        assert!(zh.starts_with("---\n\n# 写作方法论参考（完整版）"));
        assert!(zh.contains("## 六、写前自检清单"));
        let en = build_writing_methodology_section(WritingLanguage::En);
        assert!(en.starts_with("---\n\n# Writing Methodology Reference (Full Version)"));
        assert!(en.contains("## 6. Pre-Write Checklist"));
    }
}
