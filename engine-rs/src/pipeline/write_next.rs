...
async fn write_next_chapter_locked(
    ...
    // 519 号：必须在本章工件落盘（persist_chapter_artifacts）**之后**投影——
    // 此前置于落盘前，读到的是上一章的旧结构化 state，sqlite 永远缺本章
    // 摘要行（currentChapter/promises/timeline 面停在上章；TS 侧顺序为
    // saveChapter → syncNarrativeMemoryIndex，本轮活体差分坐实）。
    {
