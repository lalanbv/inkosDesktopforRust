...
fn build_fallback_memo_markdown(input: &FallbackMemoInput<'_>) -> String {
    ...
    if input.language == WritingLanguage::En {
        ...
        return [
            ...
            String::new(),
            "## Reader experience contract".to_string(),
            "- previousHandoff: the opening directly continues the previous chapter's final action and suspense without restarting the scene".to_string(),
            "- readerQuestion: carry forward the core suspense left by the outline and the previous chapter".to_string(),
            "- promisePayoff: pay off the nearest promise already supported by context".to_string(),
            "- protagonistWant: advance the protagonist's most urgent need implied by the chapter goal".to_string(),
            "- protagonistObstacle: use resistance already established by the cast and setting".to_string(),
            "- sceneTurn: land one perceptible turn in the key scene".to_string(),
            "- endingNetChange: at least one clear change in information / pressure / relationship / goal / risk by the end".to_string(),
            "- titleCandidates: fallback chapter title".to_string(),
            String::new(),
    ...
    [
        ...
        String::new(),
        "## 读者体验合同".to_string(),
        "- 开头承接：直接衔接上一章结尾的动作与悬念，不重启场景".to_string(),
        "- 读者问题：延续大纲和上一章留下的核心悬念".to_string(),
        "- 承诺兑现：兑现当前上下文已支撑的最近期承诺".to_string(),
        "- 主角欲求：按本章目标推进主角当前最紧迫的诉求".to_string(),
        "- 主角障碍：以既有人物与设定中的阻力为主".to_string(),
        "- 场景转折：在关键场景完成一次可感知的局面变化".to_string(),
        "- 章末净变化：章尾至少发生一处信息/压力/关系/目标/风险的明确变化".to_string(),
        "- 章名候选：降级章节标题".to_string(),
        String::new(),
        "## 本章 hook 账".to_string(),
