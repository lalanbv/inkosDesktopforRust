//! planner 提示词（system zh/en + user 模板 + 黄金三章指引）。
//!
//! 移植自 `packages/core/src/agents/planner-prompts.ts`（404 行）。
//! planner LLM 收 system prompt 原文 + [`build_planner_user_message`] 装配的
//! user message。输出是普通 Markdown 小节（非 YAML frontmatter、非 JSON）。
//!
//! ## 移植纪律
//! - 系统提示词与用户模板 **zh/en 双语逐字**（码点级），占位符 `{{...}}` 原样保留
//! - `replaceAll` 全量替换；`buildGoldenOpeningGuidance` 是**条件追加**（≤3 章），
//!   追加在已填充模板之后，前导 `\n\n`

use crate::utils::language::WritingLanguage;

/// 中文系统提示词。逐字移植 TS `PLANNER_MEMO_SYSTEM_PROMPT`。
pub const PLANNER_MEMO_SYSTEM_PROMPT: &str = r###"你是这本小说的创作总编，职责是为下一章产生一份 chapter_memo。你不写正文——你只规划这章要完成什么、兑现什么、不要做什么。下游写手（writer）会按你的 memo 扩写正文。
...
/// 英文系统提示词。逐字移植 TS `PLANNER_MEMO_SYSTEM_PROMPT_EN`。
pub const PLANNER_MEMO_SYSTEM_PROMPT_EN: &str = r###"You are this novel's editor-in-chief. Your job is to produce a chapter_memo for the next chapter. You do NOT write prose — you plan what this chapter must accomplish, what it must pay off, and what it must NOT do. The downstream writer expands your memo into prose.
...
/// 中文用户模板。逐字移植 TS `PLANNER_MEMO_USER_TEMPLATE`。
pub const PLANNER_MEMO_USER_TEMPLATE: &str = r##"# 第 {{chapterNumber}} 章 memo 请求
...
/// 英文用户模板。逐字移植 TS `PLANNER_MEMO_USER_TEMPLATE_EN`。
pub const PLANNER_MEMO_USER_TEMPLATE_EN: &str = r##"# Chapter {{chapterNumber}} memo request
...
pub struct PlannerUserMessageInput<'a> {
    ...
    pub previous_chapter_ending_excerpt: &'a str,
    pub recent_summaries: &'a str,
    pub current_arc_prose: &'a str,
    pub protagonist_matrix_row: &'a str,
    pub opponent_rows: &'a str,
    pub collaborator_rows: &'a str,
...
/// 语言对应系统提示词（zh 默认，en 显式）。
pub fn get_planner_memo_system_prompt(language: WritingLanguage) -> &'static str {
...
/// 语言对应用户模板。
pub fn get_planner_memo_user_template(language: WritingLanguage) -> &'static str {
    if language == WritingLanguage::En {
...
/// 装配 planner user message：模板填充 + 黄金三章指引条件追加。
pub fn build_planner_user_message(input: &PlannerUserMessageInput<'_>) -> String {
    let language = input.language;
    let template = get_planner_memo_user_template(language);
    let (yes_text, no_text) = if language == WritingLanguage::En {
        ("yes", "no")
    } else {
        ("是", "否")
    };

    let brief_block = build_brief_block(input.brief.unwrap_or(""), language);
    let chapter_context_block =
        build_chapter_context_block(input.chapter_context.unwrap_or(""), language);
    ...
    let filled = template
        ...
        .replace("{{chapter_context_block}}", &chapter_context_block)
        .replace(
            "{{previous_chapter_ending_excerpt}}",
            input.previous_chapter_ending_excerpt,
        )
        .replace("{{recent_summaries}}", input.recent_summaries)
        .replace("{{current_arc_prose}}", input.current_arc_prose)
        .replace("{{protagonist_matrix_row}}", input.protagonist_matrix_row)
        .replace("{{opponent_rows}}", input.opponent_rows)
        .replace("{{collaborator_rows}}", input.collaborator_rows)
...
/// brief 是用户的原始创作文档——「这本书是什么」的最高权威来源。
/// story_frame/volume_map 是架构师对 brief 的抽象；章节 memo 必须先尊重 brief。
/// 无 brief 返回 ""（无 brief.md 的 legacy 书）。
fn build_brief_block(brief: &str, language: WritingLanguage) -> String {
...
/// 黄金三章散文指引（chapterNumber ≤ 3 条件追加；无新 schema、无运行时分支；
/// 连贯段落而非清单）。
pub fn build_golden_opening_guidance(
    chapter_number: u32,
...
mod tests {
    ...
    fn sample_input(language: WritingLanguage) -> PlannerUserMessageInput<'static> {
        PlannerUserMessageInput {
            chapter_number: 2,
            previous_chapter_ending_excerpt: "林动握紧了玉符。",
            recent_summaries: "| 章节 |",
            current_arc_prose: "活跃支线：\n- S1",
            protagonist_matrix_row: "| 林动 | 主角本人 |",
            opponent_rows: "（暂无明确对手登场）",
            collaborator_rows: "| 乙 | 盟友 |",
    ...
    #[test]
    fn user_message_zh_fills_all_placeholders_and_appends_golden() {
        let got = build_planner_user_message(&sample_input(WritingLanguage::Zh));
        assert!(!got.contains("{{"), "占位符应全部填充：{got}");
        assert!(got.contains("# 第 2 章 memo 请求"));
        assert!(got.contains("- 是否黄金三章：是"));
        assert!(got.contains("## 用户创作 brief（原始意图——最高优先级）"));
        assert!(got.contains("## 本章用户指令（本章最高优先级）"));
        assert!(got.ends_with("禁止整段 exposition。"));
        // 黄金三章指引以 \n\n 追加在填充后的模板之后。
        assert!(got.contains("引力线。\n\n本章 memo 的 goal 字段"));
    }

    #[test]
    fn user_message_en_uses_english_template() {
        let got = build_planner_user_message(&sample_input(WritingLanguage::En));
        assert!(got.contains("# Chapter 2 memo request"));
        assert!(got.contains("- Golden opening chapter: yes"));
        assert!(got.contains("## Golden Opening Guidance — Chapter 2"));
    }

    #[test]
    fn golden_guidance_absent_after_chapter_three() {
        assert!(build_golden_opening_guidance(4, WritingLanguage::Zh).is_empty());
        assert!(!build_golden_opening_guidance(3, WritingLanguage::Zh).is_empty());

        let mut input = sample_input(WritingLanguage::Zh);
        input.chapter_number = 4;
        input.is_golden_opening = false;
        let got = build_planner_user_message(&input);
        assert!(!got.contains("黄金三章规划指引"));
        assert!(got.contains("- 是否黄金三章：否"));
    }

    #[test]
    fn brief_and_context_blocks_empty_when_missing() {
        let mut input = sample_input(WritingLanguage::Zh);
        input.brief = None;
        input.chapter_context = None;
        let got = build_planner_user_message(&input);
        assert!(!got.contains("用户创作 brief"));
        assert!(!got.contains("本章用户指令"));
        // 空块只留换行（模板 "\n\n{brief}\n{ctx}\n\n" 替换为空后连成 5 个 \n）。
        assert!(got.contains("# 第 2 章 memo 请求\n\n\n\n\n## 上一章最后一屏"));
    }

    #[test]
    fn system_prompts_are_verbatim_constants() {
        // 抽查若干 load-bearing 片段（完整性由 golden 差分守门）。
        assert!(PLANNER_MEMO_SYSTEM_PROMPT.contains("圆心法同场多视角"));
        assert!(PLANNER_MEMO_SYSTEM_PROMPT.contains("硬底线是\"揭 1 埋 1\""));
        assert!(PLANNER_MEMO_SYSTEM_PROMPT_EN.contains("Reveal 1, bury 2 (recommended)"));
        assert_eq!(get_planner_memo_system_prompt(WritingLanguage::Zh), PLANNER_MEMO_SYSTEM_PROMPT);
        assert_eq!(get_planner_memo_system_prompt(WritingLanguage::En), PLANNER_MEMO_SYSTEM_PROMPT_EN);
    }
}
