...
### 写作智能与运营面板（330~425 号，六轮对标演进）

- **失败接管链**：LLM 单模型失败按可重试白名单自动切换备用模型（次数/备用可配），接管事件在遥测与日志显式留痕
- **运行遥测**：每次 LLM 调用的元数据（agent / 模型 / 耗时 / 成败 / 接管）环形缓冲，Dashboard 面板与 `/run-log` 端点可查（只记元数据，不含正文）
- **章节目标防复读门**：规划目标与近章摘要高度重合时自动带反馈重规划一次，仍犯降级告警不阻断
- **伏笔分类**：7 类规范分类（悬念 / 情感 / 物品等）随写作链产出，伏笔池与承诺账本支持按分类筛选
- **市场雷达 → 一键开书**：雷达分析推荐卡可直接预填建书草稿
- **持久层原子写**：配置 / 索引类写盘统一 temp+rename+文件锁重试，崩溃与占用不再产生截断文件
- 配套运营面板：承诺账本（节奏债 / 紧迫度）、质量趋势、张力曲线、上下文透视（装配回放）、信息差账本、实体卡、三库资产、导演驾驶舱、备份恢复
