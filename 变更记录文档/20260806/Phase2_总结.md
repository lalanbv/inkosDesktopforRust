...
- **Tauri 命令迁移**（5 个核心命令）
  - `cmd_choose_project`: 项目选择错误
  - `cmd_check_updates`: 更新检查错误
  - `cmd_apply_engine_update`: Engine 更新错误
  - `cmd_apply_shell_update`: Shell 更新错误
  - `cmd_get_diagnostics`: 诊断信息错误

- **错误转换修复**
  - keyring::Error: 匹配 Box<dyn Error> 类型
  - 智能检测 "denied" 关键词并提供授权建议
