  ...
  private buildFallbackMemoMarkdown(input: {
    ...
    if (input.language === "en") {
      return [
        ...
        "- previousHandoff: the opening directly continues the previous chapter's final action and suspense without restarting the scene",
        "- readerQuestion: carry forward the core suspense left by the outline and the previous chapter",
        "- promisePayoff: pay off the nearest promise already supported by context",
        "- protagonistWant: advance the protagonist's most urgent need implied by the chapter goal",
        "- protagonistObstacle: use resistance already established by the cast and setting",
        "- sceneTurn: land one perceptible turn in the key scene",
        "- endingNetChange: at least one clear change in information / pressure / relationship / goal / risk by the end",
        "- titleCandidates: fallback chapter title",
        "",
    ...
    return [
      ...
      "",
      "## 读者体验合同",
      "- 开头承接：直接衔接上一章结尾的动作与悬念，不重启场景",
      "- 读者问题：延续大纲和上一章留下的核心悬念",
      "- 承诺兑现：兑现当前上下文已支撑的最近期承诺",
      "- 主角欲求：按本章目标推进主角当前最紧迫的诉求",
      "- 主角障碍：以既有人物与设定中的阻力为主",
      "- 场景转折：在关键场景完成一次可感知的局面变化",
      "- 章末净变化：章尾至少发生一处信息/压力/关系/目标/风险的明确变化",
      "- 章名候选：降级章节标题",
      "",
      "## 本章 hook 账",
