...
## Task 4: 三层配置合并逻辑（0GC 优化）

**Files:**
- Create: `src-tauri/src/config/merge.rs`
- Modify: `src-tauri/src/config/mod.rs`

**Interfaces:**
- Consumes: Task 1-3 的 `AppConfig`
- Produces:
  - `pub struct ConfigManager { system, workspace, project, merged }`
  - `impl ConfigManager::new() -> Self`
  - `impl ConfigManager::set_workspace(&mut self, workspace_cfg: AppConfig)`
  - `impl ConfigManager::set_project(&mut self, project_cfg: AppConfig)`
  - `impl ConfigManager::merged(&self) -> &AppConfig`

**Design:**
- 三层配置：System（默认）→ Workspace → Project
- 合并策略：`Cow<'_, T>` 避免不必要的克隆
- 优先级：Project > Workspace > System
- 字段级合并：每个字段独立覆盖
...
```bash
cargo test --lib config::merge::tests --no-fail-fast
```
...
```bash
cargo test --lib config::merge::tests
```
...
```bash
git add src-tauri/src/config/
git commit -m "feat(config): Task 4 - 三层配置合并逻辑（0GC 优化）

- merge.rs: ConfigManager 三层合并
- 配置层级：System → Workspace → Project
- 字段级覆盖：每个字段独立合并
- 懒加载：仅在 dirty 时重新计算
- API:
  - new(): 使用系统默认值
  - set_workspace/set_project: 设置层级配置
  - clear_workspace/clear_project: 清除层级配置
  - merged(): 获取合并结果（懒加载）
...
**Interfaces:**
- Consumes: Task 1-4 的 `ConfigManager`
- Produces:
  - `#[tauri::command] async fn get_config() -> Result<AppConfig, String>`
  - `#[tauri::command] async fn update_config(layer: ConfigLayer, config: AppConfig) -> Result<(), String>`
  - `#[tauri::command] async fn reset_config(layer: ConfigLayer) -> Result<(), String>`

**Design:**
- 全局状态：`Arc<Mutex<ConfigManager>>` 注入 Tauri State
- 命令：get/update/reset 三个操作
- 错误处理：anyhow → String（Tauri 序列化限制）
...
mod tests {
    ...
    async fn test_get_config_default() {
        let state = AppState {
            config: Arc::new(Mutex::new(ConfigManager::new())),
        };
    ...
    #[tokio::test]
    async fn test_update_workspace_config() {
        let state = AppState {
            config: Arc::new(Mutex::new(ConfigManager::new())),
        };
        ...
        let result = update_config(state.clone(), ConfigLayer::Workspace, new_cfg).await;
        assert!(result.is_ok());
        
        let merged = get_config(state).await.unwrap();
        assert_eq!(merged.logging.level, "debug");
    }
    ...
    async fn test_reset_config() {
        ...
        let mut new_cfg = AppConfig::default();
        new_cfg.logging.level = "debug".to_string();
        update_config(state.clone(), ConfigLayer::Workspace, new_cfg).await.unwrap();
        
        // 重置工作区配置
        let result = reset_config(state.clone(), ConfigLayer::Workspace).await;
        assert!(result.is_ok());
...
```bash
cargo test --lib commands::config::tests --no-fail-fast
```
...
use crate::config::{AppConfig, ConfigLayer, ConfigManager};
use std::sync::Arc;
use tokio::sync::Mutex;
...
```bash
cargo test --lib commands::config::tests
```
...
git commit -m "feat(config): Task 5 - Tauri 命令实现

- commands/config.rs: 三个 Tauri 命令
  - get_config: 获取合并后配置
  - update_config: 更新层级配置（禁止系统层）
  - reset_config: 重置层级配置（禁止系统层）
- AppState: Arc<Mutex<ConfigManager>> 全局状态
- main.rs: 注册命令 + 初始化状态
